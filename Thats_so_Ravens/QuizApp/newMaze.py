...
class Collider(threading.Thread):
    ...
    def collision(self):
        if pygame.sprite.collide_mask(self.player, self.current):
            self.player.kill()

    def box(self):
        if pygame.sprite.collide_rect(self.player, self.current.next):
            self.current = self.current.next
        elif pygame.sprite.collide_rect(self.player,self.current.prev):
